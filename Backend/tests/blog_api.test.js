...
test("delete a post", async () => {
  ...
  const response = await api.delete(`/api/blogs/${blogToDelete}`).set('Authorization', `Bearer ${token}`);
...
test("updating a post", async () => {
  ...
  const response = await api
    ...
    .send(newblog);
  finalblog = await api.get(`/api/blogs/${blogToUpdate.id}`);
  assert.strictEqual(24, finalblog.body.likes);
});
